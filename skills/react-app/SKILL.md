...
标准 CRUD 的列表请求、分页、增删改 Mutation、缓存刷新和新增/编辑弹窗状态优先交给 `useCrud`，页面不要重复维护一套。

普通后台 CRUD 的筛选默认放页面 local state；只有明确需要分享链接、刷新保留或浏览器前进/后退恢复时才使用 URL Search Params。
...
- 标准 CRUD 分页是否交给 `useCrud`，页面只保留业务筛选？
- 普通 CRUD 是否无明确需求却使用 URL 保存筛选和分页？
