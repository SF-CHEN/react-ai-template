...
- [x] 7. 加入 Dashboard、用户 CRUD、Query、Store、Form、Table Demo
...
生成环境无法稳定访问 npm registry，因此第 12 项无法在这里真实安装依赖后执行。请在可访问 npm 的本地环境运行：
...
当前已完成不依赖 `node_modules` 的检查：

- 所有 TS/TSX 文件 TypeScript parser 解析通过
